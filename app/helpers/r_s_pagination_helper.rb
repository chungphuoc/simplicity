module RSPaginationHelper
    ...
    def rs_page_links( rspager, param_hash={} )
        ...
        internal_pages.each do | page_num |
            ...
            html << "&bull;&bull;" if (page_num-last_page_num)>1
    ...
    def sorting_header( rspager, title, order_by_key, param_hash={} )
        param_hash[rspager.name] = rspager.current_page;
        param_hash[:action] = params[:action];
        param_hash[:controller] = params[:controller];
        param_hash[:order_by] = order_by_key;
        if ( order_by_key==rspager.pagee.order_by )
            param_hash[:descending] = (!rspager.pagee.descending?).to_s;
        end
        link_to( title, param_hash );
    end
