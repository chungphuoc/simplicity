class BusinessBuildingController < ApplicationController
    ...
    def index
        ...
        qry = UserRelatedMtReqsQuery.new(@user);
        qry.clear_states();
        pgr = RSPager.new( qry );
        pgr.page_size=9;
    ...
    def user_destroy
        if request.post?
            begin
                ...
                if (cur_user == @user )
                    add_error "USER CANNOT DELETE HIMSELF"
                    user_list
    ...
    def mt_req_list
        ...
        when MT_QUERY_TYPE_PENDING:
            @pgr = get_state( MT_QUERY_TYPE_PENDING ) do
                ...
                qry.add_state( MaintenanceRequest::OPEN );
                qry.add_state( MaintenanceRequest::IN_PROGRESS );
                RSPager.new( qry )
            end
