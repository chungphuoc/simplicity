...
class AnnouncementsControllerTest < Test::Unit::TestCase
  ...
  def test_show
    ...
    assert_not_nil assigns(:announcements)
    assert assigns(:announcements).valid?
  end
  ...
  def test_create
    num_announcements = Announcements.count
    ...
    assert_equal num_announcements + 1, Announcements.count
  end
  ...
  def test_destroy
    assert_not_nil Announcements.find(1)
